...
import { addUser, setKeywordsForSearching } from '@/store/slicing'
    ...
    <div className='w-full h-20 bg-amazon_blue text-lightText sticky top-0 z-50 '>
      <div className='w-full h-full gap-1 mx-auto inline-flex items-center justify-between px-4'>
        ...
        <Link href={'/Favorite'} className='relative text-xs px-2 border border-transparent hover:border-white cursor-pointer duration-300 flex flex-col  justify-center h-[70%]'>
          ...
          <span className='absolute text-xs text-amazon_yellow h-4 w-4 flex items-center justify-center top-2 right-0 font-semibold border-[1px] px-2'>{favData.length && favData.length}</span>
        </Link>

        <Link href='/Cart' className='px-2 border border-transparent hover:border-white cursor-pointer duration-300 flex items-center justify-center h-[70%] relative'>
          {/* <HoverDiving> */}
